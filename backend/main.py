...
# ---------------------------------------------------------
# NATIVE DATABASE CLIENT (Neon PostgreSQL)
# ---------------------------------------------------------
from db import db, db as prisma
...
async def lifespan(app: FastAPI):
    ...
    await db.connect()
    logger.info("Connected to Neon PostgreSQL via native database adapter")
    
    # Auto-seed if empty
    count = await db.recoveryevent.count()
    if count == 0:
